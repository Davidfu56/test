...
class UserAuthView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user:
            ...
            return WResponse({'token': jwt.encode(payload, SECRET_KEY)}, status=OK)
        else:
            return WResponse('用户名或密码错误!', status=BAD)
...
class UserInfoView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return WResponse(data, status=OK)
        else:
            return WResponse('请登录后访问!', status=FORBIDDEN)
...
class UserBuildMenuView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return WResponse(menu_data, status=OK)
        else:
            return WResponse('请登录后访问!', status=FORBIDDEN)
...
class UserViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return WResponse(serializer.data, status=CREATED, headers=headers)
    ...
    def set_password(self, request, pk=None):
        ...
        if 'admin' in perms or 'user_all' in perms or request.user.is_superuser:
            ...
            if new_password1 == new_password2:
                ...
                user.save()
                return WResponse('密码修改成功!')
            else:
                return WResponse('新密码两次输入不一致!', status=status.HTTP_400_BAD_REQUEST)
        else:
            ...
            if check_password(old_password, user.password):
                ...
                return WResponse('旧密码错误!', status=status.HTTP_400_BAD_REQUEST)
