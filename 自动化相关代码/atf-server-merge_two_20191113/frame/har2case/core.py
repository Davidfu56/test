...
class HarParser(object):
    ...
    def _make_apiinfo(self):
        """
        Extract info from HAR file and prepare for api.
        :return: dict
        """
        logging.debug("Extract info from HAR file and prepare for api.")
        log_entries = utils.load_har_log_entries(self.har_file_path)
        if len(log_entries)==1:
            return self._prepare_teststep(log_entries[0])


    def gen_testcase_tofile(self, file_type="JSON", fmt_version="v2", out_type="CASE"):
    ...
    def gen_testcase_tojson(self, out_type=None):
        logging.info("Start to generate testcase.")
        if out_type == "API":
            testcase = self._make_apiinfo()
        else:
            testcase = self._make_testcase("v2")
        logging.debug("prepared testcase: {}".format(testcase))
        return testcase